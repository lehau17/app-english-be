...
export class MakeupRequestService {
  ...
  async createRequest(
    ...
    await this.notifyAdmin(sessionId, studentId, request.id, dto.reason);
  ...
  private async notifyTeacher(
    ...
    try {
      ...
      this.logger.log(
        `Sent makeup request notification to teacher ${session.classroom.teacherId}`,
      );
    } catch (error) {
      this.logger.error(
        `Failed to send makeup request notification: ${error.message}`,
        error.stack,
      );
  ...
  private async notifyAdmin(
    ...
    try {
      // Get session with classroom info
      const session = await this.prisma.classroomSession.findUnique({
        where: { id: sessionId },
        include: {
          classroom: {
      ...
      if (!session) {
        this.logger.warn(
          `Session ${sessionId} not found for admin notification`,
        );
        return;
      }
      ...
      this.logger.log(
        `Sent makeup request notification to ${admins.length} admin(s)`,
      );
    } catch (error) {
      this.logger.error(
        `Failed to send makeup request notification to admins: ${error.message}`,
        error.stack,
      );
      // Don't throw - notification failure shouldn't block request creation
    }
