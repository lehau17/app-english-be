...
import { AssignmentType, NotificationChannel, NotificationType, SessionRescheduleRequestStatus } from '@prisma/client';
...
export class RescheduleRequestService {
    ...
    async createRequest(
        ...
        if (session.instructorId !== requestedById) {
            throw new ForbiddenException('Chỉ giáo viên của buổi học mới có thể tạo yêu cầu dời lịch');
        }

        // Check if there's already a pending request for this session
        const existing = await this.rescheduleRequestRepository.findPendingBySession(sessionId);
        if (existing) {
            throw new ConflictException('Đã có yêu cầu dời lịch đang chờ duyệt cho buổi học này');
        }

        // Validate time window (7:00 AM - 10:00 PM)
        const timeWindowValidation = this.validateTimeWindow(dto.newStartTime, dto.newEndTime);
        if (!timeWindowValidation.isValid) {
            this.logger.warn(`Time window validation failed: ${timeWindowValidation.message}`, {
                sessionId,
                requestedById,
        ...
        if (!periodValidation.isValid) {
            this.logger.warn(`Classroom period validation failed: ${periodValidation.message}`, {
                sessionId,
        ...
        if (conflicts.hasConflicts) {
            throw new ConflictException(
                `Không thể dời lịch: ${conflicts.message}`,
            );
        }
        ...
        await this.notifyAdmin(sessionId, requestedById, request.id, dto.reason, session);
    ...
    private async notifyAdmin(
        ...
        try {
            ...
            this.logger.error(`Failed to send reschedule request notification: ${error.message}`, error.stack);
     ...
     */
    async getSessionRequests(sessionId: string, query: QueryRescheduleRequestDto) {
    ...
    async getPendingRequestBySession(sessionId: string, teacherId: string) {
        const request = await this.rescheduleRequestRepository.findPendingBySession(sessionId);
    ...
    async updateRequest(
        ...
        // Verify teacher owns the request
        if (request.requestedById !== teacherId) {
            throw new ForbiddenException('Chỉ người tạo yêu cầu mới có thể chỉnh sửa');
        }
        ...
        if (updateData.newStartTime || updateData.newEndTime) {
            ...
            const timeWindowValidation = this.validateTimeWindow(newStartTime, newEndTime);
            if (!timeWindowValidation.isValid) {
                this.logger.warn(`Time window validation failed: ${timeWindowValidation.message}`, {
                    requestId,
                    teacherId,
        ...
        // Update the request
        const updatedRequest = await this.rescheduleRequestRepository.update(requestId, updateData);

        this.logger.log(`Updated reschedule request ${requestId} by teacher ${teacherId}`);
    ...
    private async sendRescheduleNotifications(request: any) {
        if (!this.kafkaService) {
            this.logger.warn('KafkaService not available, skipping email notifications');
            return;
        }
        ...
        try {
            ...
            if (!session) {
                this.logger.warn(`Session ${request.sessionId} not found for notifications`);
                return;
            }

            const teacherName = session.instructor?.displayName
            ...
            const newStartTime = new Date(request.newStartTime).toLocaleString('vi-VN', {
                weekday: 'long',
                day: '2-digit',
                month: '2-digit',
                year: 'numeric',
            ...
            for (const enrollment of classroomStudents) {
                ...
                for (const parent of parents) {
                    ...
                    const childName = student.displayName
                        || `${student.firstName || ''} ${student.lastName || ''}`.trim()
                        || 'Con bạn';
            ...
            this.logger.error(`Failed to send reschedule notifications: ${error.message}`, error.stack);
        }
    }
    ...
    private async findLinkedParents(studentId: string): Promise<Array<{ id: string; email: string | null }>> {
        try {
            ...
            return relations.map((r) => r.parent).filter((p) => p !== null && p.email !== null);
        } catch (error) {
    ...
    private async validateExamDate(
        ...
        const { assignments } = await this.assignmentRepository.findAssignmentsByClassroom(
            classroomId,
        );
        ...
        for (const exam of examAssignments) {
            ...
            if (
                newDateOnly.getTime() === examStartDateOnly.getTime()
            ) {
                const examTypeName =
                    exam.type === AssignmentType.MIDTERM_EXAM
                        ? 'giữa kỳ'
                        : 'cuối kỳ';
                throw new BadRequestException(
            ...
            if (exam.dueDate) {
                ...
                if (
                    newDateOnly.getTime() === examEndDateOnly.getTime()
                ) {
                    const examTypeName =
                        exam.type === AssignmentType.MIDTERM_EXAM
                            ? 'giữa kỳ'
    ...
    private async checkAvailabilityConflicts(
        ...
        if (studentConflicts.length > 0) {
            const conflictList = studentConflicts
                ...
                .join(', ');
            const moreCount = studentConflicts.length > 3
    ...
    private async updateSessionTime(
        ...
        this.logger.log(`Updated session ${sessionId} time to ${newStartTime} - ${newEndTime}`);
    }
