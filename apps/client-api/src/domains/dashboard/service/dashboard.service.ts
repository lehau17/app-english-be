...
export class DashboardService {
  ...
  private async getRevenueTrend(): Promise<
  ...
  private async getTopCourses(): Promise<
    Array<{
      id: string;
      title: string;
      enrollments: number;
  ...
  > {
    ...
    return courses.map((course) => ({
      ...
      revenue: course.transactions.reduce(
...
Bắt đầu response của bạn bằng dấu { và kết thúc bằng dấu }.`;
  ...
  async getClassAIAnalytics(
    ...
    const totalScore = classProgress.reduce(
      (sum, p) => sum + (p.score || 0),
      0,
    );
    ...
    const strugglingStudents: StrugglingStudent[] = Object.entries(
      ...
      .map(([id, data]) => ({
        ...
        averageScore:
          data.activities > 0 ? data.totalScore / data.activities : 0,
        issue: '',
      }))
...
Hãy làm lại phân tích với dữ liệu trên và trả về CHỈ JSON hợp lệ, không có gì khác.`;
            ...
            currentResponse =
              await this.geminiService.generateJSONResponse(retryPrompt);
            console.log(
