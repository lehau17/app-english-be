...
export class SuccessRateCalculatorService {
  ...
  async getTopicSuccessRates(userId: string): Promise<TopicSuccessRate[]> {
    ...
    const successRateData: TopicSuccessRate[] = topicProgress.map(
      (progress) => {
        ...
        return {
          ...
          successRateInverseFactor: Math.max(
            0,
            Math.min(1, successRateInverseFactor),
          ),
        };
      },
    );
  ...
  async getTopicSuccessRateByCategory(
    userId: string,
    category: string,
  ): Promise<TopicSuccessRate | null> {
