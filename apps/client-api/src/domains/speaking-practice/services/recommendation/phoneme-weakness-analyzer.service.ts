...
export class PhonemeWeaknessAnalyzerService {
  ...
  async getUserWeakPhonemes(userId: string): Promise<PhonemeWeakness[]> {
    ...
    const phonemeMap = new Map<
      string,
      { errorCount: number; correctCount: number }
    >();
    ...
    const weaknessData: PhonemeWeakness[] = Array.from(
      phonemeMap.entries(),
    ).map(([phoneme, stats]) => {
      const totalAttempts = stats.errorCount + stats.correctCount;
      const errorRate =
        totalAttempts > 0 ? stats.errorCount / totalAttempts : 0;
    ...
    const filteredWeaknesses = weaknessData
      .filter(
        (w) => w.errorRate >= recommendationConfig.minErrorRateForWeakness,
      )
      .sort((a, b) => b.errorRate - a.errorRate);

    // Return top N weak phonemes
    return filteredWeaknesses.slice(
      0,
      recommendationConfig.topWeakPhonemesCount,
    );
  }
}
