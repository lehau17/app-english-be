...
export class PronunciationAssessmentService {
  ...
  async assessPronunciation(
    ...
    try {
      ...
      throw new Error(
        `Pronunciation assessment failed: ${error.message}`,
      );
    }
  }
  ...
  private async extractWordAssessments(
    ...
    for (const wordInfo of alternative.words) {
      ...
      const phonemes = this.estimatePhonemes(word, accuracyScore, startTime, duration);
   ...
   */
  private calculateFluencyScore(words: WordAssessment[], confidence: number): number {
  ...
  private generateRecommendations(
    ...
    if (accuracyScore < 60) {
      recommendations.push('Focus on pronunciation accuracy. Slow down and enunciate each word clearly.');
    }

    if (fluencyScore < 60) {
      recommendations.push('Improve fluency by practicing speaking at a natural pace without long pauses.');
    }

    if (recommendations.length === 0) {
      recommendations.push('Great pronunciation! Keep practicing to maintain your skills.');
    }
  ...
  private createEmptyFeedback(): PronunciationFeedback {
    return {
      ...
      words: [],
      problematicPhonemes: [],
      recommendations: ['No speech detected. Please try again and speak clearly.'],
      durationSec: 0,
      wordsPerMinute: 0,
    };
  }
