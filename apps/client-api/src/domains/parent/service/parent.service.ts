...
export class ParentService {
  ...
  async getActivities(
    ...
    try {
      ...
      if (options.status) {
        const stateFilters = this.mapUiStatusToStates(options.status);
        if (stateFilters?.length) {
          where.state =
            stateFilters.length === 1 ? stateFilters[0] : { in: stateFilters };
        }
      }
  ...
  private mapUiStatusToStates(status: string): ProgressState[] | null {
    const map: Record<string, ProgressState[]> = {
      completed: [ProgressState.done, ProgressState.mastered],
      in_progress: [ProgressState.in_progress],
      failed: [ProgressState.not_started, ProgressState.review_needed],
      skipped: [ProgressState.review_needed],
    };
