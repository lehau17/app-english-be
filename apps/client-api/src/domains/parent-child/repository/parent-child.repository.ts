...
export class ParentChildRepository {
    ...
    async findById(parentId: string, childId: string): Promise<ParentChild | null> {
        return this.prisma.parentChild.findUnique({ where: { parentId_childId: { parentId, childId } } });
    }

    async delete(parentId: string, childId: string): Promise<ParentChild> {
        return this.prisma.parentChild.delete({ where: { parentId_childId: { parentId, childId } } });
    }
