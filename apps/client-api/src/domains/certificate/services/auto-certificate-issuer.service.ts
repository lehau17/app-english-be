...
export class AutoCertificateIssuerService {
    ...
    async checkAndIssueCertificate(
        ...
        try {
            ...
            // Get course progress
            const progress = await this.calculateCourseProgress(studentId, courseId, classroomId);
    ...
    private async calculateCourseProgress(
        ...
        const totalHours = completedAssignments.reduce((total, assignment) => {
            ...
            if (submission.submittedAt && submission.startedAt) {
                const hours = (submission.submittedAt.getTime() - submission.startedAt.getTime()) / (1000 * 60 * 60);
                return total + hours;
            }
            return total;
        }, 0);
    ...
    async batchIssueCertificates(courseId?: string): Promise<void> {
        this.logger.log(`Starting batch certificate issuance${courseId ? ` for course ${courseId}` : ''}`);
