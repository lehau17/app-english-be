...
import { CertificateRequirementType, CertificateTemplate } from '@prisma/client';
...
export class CertificateTemplateService {
    ...
    async createDefaultTemplate(courseId: string): Promise<CertificateTemplate> {
        this.logger.log(`Creating default certificate template for course ${courseId}`);

        // Check if template already exists
        const existing = await this.templateRepo.findByCourseId(courseId);
        if (existing) {
            throw new ConflictException('Certificate template already exists for this course');
        }
        ...
        this.logger.log(`Created certificate template ${template.id} for course ${courseId}`);
        return template;
    }
    ...
    async create(dto: CreateCertificateTemplateDto): Promise<CertificateTemplate> {
        ...
        const template = await this.templateRepo.create({
            ...
            logoUrl: dto.logoUrl,
            requirementType: dto.requirementType || CertificateRequirementType.course_completion,
    ...
    async getTemplateByCourseId(courseId: string): Promise<CertificateTemplate> {
        ...
        if (!template) {
            throw new NotFoundException(`Certificate template not found for course ${courseId}`);
        }
        return template;
    }
    ...
    async getTemplateById(id: string): Promise<CertificateTemplate> {
        ...
        if (!template) {
            throw new NotFoundException(`Certificate template not found with id ${id}`);
        }
        return template;
    }
     ...
     * Update template
     */
    async update(id: string, dto: UpdateCertificateTemplateDto): Promise<CertificateTemplate> {
    ...
    async findAll(params?: {
        ...
        const where = params?.isActive !== undefined ? { isActive: params.isActive } : undefined;
