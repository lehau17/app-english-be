...
export class VideoProcessingService {
  ...
  async processVideo(
    ...
    try {
      ...
      const audioResult =
        await this.audioExtractionService.extractToMp3(tempVideoPath);
      ...
      const transcriptionMode =
        this.configService.get<string>('TRANSCRIPTION_SERVICE') || 'whisper';

      // Try Google STT first (faster), fallback to Whisper
      if (
        transcriptionMode === 'google' &&
        this.googleTranscriptionService.isAvailable()
      ) {
        try {
          this.logger.log(
            'Step 4/5: Transcribing with Google Speech-to-Text...',
          );

          const transcriptionResult =
            await this.googleTranscriptionService.transcribe(
          ...
          this.logger.warn(
            'Google STT error, skipping transcription:',
            error.message,
          );
        }
      } else if (transcriptionMode === 'whisper') {
        const whisperEnabled =
          this.configService.get<string>('ENABLE_WHISPER_TRANSCRIPTION') !==
          'false';

        if (whisperEnabled) {
          try {
            this.logger.log(
              'Step 4/5: Transcribing with Whisper (may take several minutes)...',
            );

            const transcriptionResult = await this.whisperService.transcribe(
              audioResult.audioPath,
            );
        ...
        this.logger.log(
          'Transcription disabled (set TRANSCRIPTION_SERVICE=google or TRANSCRIPTION_SERVICE=whisper to enable)',
        );
      }
