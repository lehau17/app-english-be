...
export class AiPodcastRecommenderService {
    ...
    private async analyzeUserProfile(userId: string): Promise<UserProfile> {
        ...
        const recentCategories = [
            ...new Set(
                attempts.slice(0, 10).map((a) => a.podcast.category).filter(Boolean),
            ),
        ];
        const recentDifficulties = [
            ...new Set(
                attempts.slice(0, 10).map((a) => a.podcast.difficulty).filter(Boolean),
            ),
        ];
        ...
        } else if (totalScore >= 70 && recentDifficulties.includes('intermediate')) {
...
- Return valid JSON only, no additional text`;
     ...
     */
    private buildUserProfileSummary(
        profile: UserProfile,
    ): UserProfileSummaryDto {
        return {
