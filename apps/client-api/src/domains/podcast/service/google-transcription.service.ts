...
export class GoogleTranscriptionService {
    ...
    constructor(private readonly configService: ConfigService) {
        ...
        if (credentialsPath && fs.existsSync(credentialsPath)) {
            ...
            this.enabled = false;
            this.logger.warn('Google Speech-to-Text disabled: GOOGLE_APPLICATION_CREDENTIALS not set or file not found');
            this.logger.warn('Set GOOGLE_APPLICATION_CREDENTIALS=/path/to/service-account-key.json to enable');
        }
    }
    ...
    async transcribe(
        ...
        if (!this.enabled) {
            return {
                ...
                duration: 0,
                success: false,
                error: 'Google Speech-to-Text not configured. Set GOOGLE_APPLICATION_CREDENTIALS.',
            };
        }
        ...
        try {
            ...
            this.logger.log(`Transcription completed in ${processingTime.toFixed(2)}s`);
            ...
            if (error.code === 7) {
                errorMessage = 'Permission denied. Check GOOGLE_APPLICATION_CREDENTIALS.';
    ...
    async transcribeLongAudio(
        ...
        try {
            ...
            this.logger.log('Long running operation started, waiting for completion...');
