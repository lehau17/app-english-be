...
export class AudioExtractionService {
  ...
  async extractToMp3(
    ...
    const audioPath = outputPath || videoPath.replace(/\.\w+$/, '.mp3');
  ...
  async extractToWav(
    ...
    const audioPath = outputPath || videoPath.replace(/\.\w+$/, '.wav');

    this.logger.log(`Extracting audio (WAV) from ${videoPath} to ${audioPath}`);
