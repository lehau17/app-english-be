...
export class PaymentResponseData {
  ...
  @ApiProperty({ description: 'Nhà cung cấp thanh toán', enum: PaymentProvider, example: PaymentProvider.vnpay })
  provider: PaymentProvider;

  @ApiProperty({ description: 'Loại giao dịch', enum: TransactionType, example: TransactionType.course_purchase })
  type: TransactionType;

  @ApiProperty({ description: 'Mã tham chiếu VNPay', example: 'ORDER_20241001_123456' })
  vnpayTxnRef: string;

  @ApiProperty({ description: 'Mã giao dịch (transaction id)', example: 'transaction-uuid' })
  transactionId: string;

  @ApiProperty({ description: 'Mã đơn hàng', example: 'ORDER_20241001_123456' })
  orderId: string;

  @ApiProperty({ description: 'Thời gian tạo', example: '2024-01-01T00:00:00Z' })
  createdAt: Date;
}
...
export class PaymentResponseDto {
  ...
  @ApiProperty({ description: 'Thông báo phản hồi', example: 'Tạo link thanh toán thành công' })
  message: string;
