...
export class PrivateAssignmentController {
    ...
    @ApiOperation({
        summary: 'Get assignment submission details for grading (Teacher/Admin only)',
        description:
            'Lấy chi tiết bài nộp bao gồm câu trả lời, học sinh, bài tập. ' +
            'Chỉ giáo viên của lớp hoặc admin mới có quyền xem.',
    })
    @ApiResponse({
        status: 200,
    ...
    async getSubmissionDetails(
        ...
        return this.assignmentService.getSubmissionDetails(
            submissionId,
            teacherUserId,
        );
    }
    ...
    @ApiOperation({
        summary: 'Grade an assignment submission (Teacher only)',
        description:
            'Chấm điểm và gửi phản hồi cho bài nộp của học sinh. ' +
    ...
    @ApiResponse({ status: 400, description: 'Invalid grade or data' })
    @ApiResponse({ status: 403, description: 'Forbidden - No permission' })
    @ApiResponse({ status: 404, description: 'Submission not found' })
    async gradeSubmissionNew(
        ...
        return this.assignmentService.gradeSubmissionNew(
            submissionId,
            dto.grade,
            dto.feedback || null,
            teacherUserId,
