...
export class AssignmentImportService {
  ...
  generateTemplate(): Buffer {
    ...
    const assignmentInfoData = [
      ...
      ['title', 'Complete English Practice Assignment', 'Assignment title', 'Yes'],
      [
        'description',
    ...
    const instructionsData = [
      ...
      ['   ⚠️  Only "title" is REQUIRED, other fields are optional'],
      ...
      [''],
      ['   🗣️ PRONUNCIATION (Phrases to practice, multiple allowed)'],
      ...
      ['   ⏱️  timeLimit: Time limit in minutes'],
      ...
      ['   🗣️ PRONUNCIATION: Add multiple rows with SAME title, different phrases'],
    ...
    for (const [groupKey, rows] of activityGroups.entries()) {
      try {
        ...
        if (type === 'reading') {
          ...
          activity = this.parseActivityRow(firstRow.row, headerMap, firstRow.rowNumber);
        }
  ...
  private parsePronunciationActivity(
    ...
    if (phrases.length === 0) {
      throw new Error('At least one phrase is required for pronunciation activities');
    }
