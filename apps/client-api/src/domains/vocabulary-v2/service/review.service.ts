...
export class ReviewService {
  ...
  async startSession(
    ...
    const terms: VocabularyTermResponseDto[] = [];
    let dueCards = 0;
    let newCards = 0;
  ...
  async submitReview(
    ...
    for (const review of reviews) {
      ...
      const nextSchedule = this.srsService.calculateNextReview(
        currentProgress,
        quality,
      );
      ...
      await this.repository.upsertProgress(
        ...
        termId,
        {
          ...
          wrongCount:
            quality < 3
              ? (currentProgress?.wrongCount || 0) + 1
  ...
  async getStats(userId: string, listId?: string): Promise<ReviewStatsDto> {
    ...
    const progressList = await this.repository.findUserProgress(userId, {
      listId,
    });
    ...
    const { currentStreak, longestStreak } =
      this.srsService.calculateStreak(reviewDates);
    ...
    if (streakMilestones.includes(currentStreak)) {
      try {
        ...
        this.logger.log(
          `Sent streak notification to user ${userId} (${currentStreak} days)`,
        );
      } catch (error) {
  ...
  async resetUnitProgress(
    ...
    this.logger.log(
      `Reset ${deletedCount} progress records for unit ${unitId}`,
    );
