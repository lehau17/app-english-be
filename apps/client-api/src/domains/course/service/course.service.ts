...
export class CourseService {
  ...
  async create(dto: CreateCourseDto, currentUserId: string): Promise<Course> {
    ...
    if (pendingMediaExtractions.length > 0) {
      ...
      this.extractMediaFromActivities(
        ...
        this.logger.error(`Error in media extraction: ${error.message}`, error);
      });
    }
        ...
        for (const lessonDto of dto.lessons) {
          const lesson = await tx.lesson.create({
            data: {
              ...
              orderNo: lessonDto.orderNo,
              difficulty:
                lessonDto.difficulty ?? dto.difficulty ?? course.difficulty,
              estimatedTime: lessonDto.estimatedTime ?? undefined, // minutes
              isLocked: lessonDto.isLocked ?? true,
              objectives: lessonDto.objectives ?? [],
            },
  ...
  private async getCoursesForExport(query: FilterCourseRequestDto) {
    ...
    const ALLOWED_SORT_BY = new Set<
      keyof Prisma.CourseOrderByWithRelationInput
    >(['orderNo', 'createdAt', 'price', 'rating', 'title']);
