...
export class SessionScheduleController {
  constructor(private readonly sessionScheduleService: SessionScheduleService) {}

  @Get()
  @ApiOperation({ summary: 'Lấy lịch trình các buổi học của một khóa học' })
  @ResponseMessage('Lịch trình buổi học được lấy thành công')
  async getSessionSchedules(@Param('courseId', new ParseUUIDPipe()) courseId: string) {
    return this.sessionScheduleService.findByCourseId(courseId);
  }

  @Put()
  @ApiOperation({ summary: 'Cập nhật toàn bộ lịch trình các buổi học của một khóa học' })
  ...
  async updateSessionSchedules(
    ...
    return this.sessionScheduleService.createSessionSchedules(courseId, schedules);
  }
  ...
  async updateSessionSchedule(
    ...
    return this.sessionScheduleService.updateSessionSchedule(courseId, sessionNumber, dto);
  }
}
