...
export class ApiSearchTool extends StructuredTool {
  name = 'api_search';
  description =
    'Tìm endpoint nội bộ theo câu tự nhiên. Trả về danh sách method+path+params phù hợp nhất.';
  schema = z.object({
    query: z
      .string()
      .describe('Mô tả tác vụ cần gọi API, vd: "liệt kê 10 học viên mới nhất"'),
