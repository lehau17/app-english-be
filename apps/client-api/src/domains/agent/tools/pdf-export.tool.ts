...
`;
    ...
    private async downloadFonts() {
        try {
            ...
            const regularResponse = await axios.get(FONT_URLS.regular, { responseType: 'arraybuffer', timeout: 30000 });
            await fs.writeFile(regularPath, Buffer.from(regularResponse.data));

            // Download bold font
            const boldResponse = await axios.get(FONT_URLS.bold, { responseType: 'arraybuffer', timeout: 30000 });
    ...
    async _call(input: string): Promise<string> {
        try {
            ...
            const fontNameRegular = fontRegular.endsWith('.ttf') ? 'Regular' : fontRegular;
                ...
                for (const chart of charts) {
                    try {
                        ...
                        if (chartImageBuffer) {
                            ...
                            // Add chart title
                            doc.fontSize(12).font(fontNameBold).text(chart.title || 'Chart', { align: 'center' });
                            doc.moveDown(0.5);
