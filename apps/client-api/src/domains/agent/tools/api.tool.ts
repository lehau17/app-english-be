...
export class ApiTool extends StructuredTool {
  name = 'call_api';
  description = 'Gọi API nội bộ qua method+path (không cần operationId). Tự gắn Authorization từ RequestContext.';
  schema = z.object({
    method: z.enum(['GET', 'POST', 'PUT', 'PATCH', 'DELETE']).describe('HTTP method'),
    path: z.string().describe('Đường dẫn đúng như trong Swagger, vd: /students/{id}'),
  ...
  async _call(input: z.infer<typeof this.schema>): Promise<string> {
    try {
      const { method, path, pathParams = {}, query = {}, body, headers = {} } = input;

      // 1) Chặn ghi nếu chưa bật
      if (!SAFE_METHODS.has(method) && !ALLOW_WRITE) {
        return JSON.stringify({ success: false, error: `Method ${method} bị chặn (chưa bật AGENT_ALLOW_WRITE=true)` });
      }
      ...
      if (!exists) {
        return JSON.stringify({ success: false, error: `Không tìm thấy endpoint ${method} ${path} trong Swagger` });
      }
      ...
      const fallback = process.env.AGENT_FALLBACK_BEARER ? `Bearer ${process.env.AGENT_FALLBACK_BEARER}` : '';
      const authHeader = headers['authorization'] || headers['Authorization'] || ctxAuth || fallback;

      const cfg: AxiosRequestConfig = {
        url, method: method as any, params: query, data: body, timeout: 10000,
        headers: { 'Content-Type': 'application/json', ...(authHeader ? { Authorization: authHeader } : {}), ...headers },
      };
      ...
      return JSON.stringify({ success: false, error: `API call error: ${e?.message}` });
    }
  }
}
