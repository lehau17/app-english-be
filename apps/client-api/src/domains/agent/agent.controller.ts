...
import {
  ContinueGuestChatDto,
  CreateGuestChatDto,
  GetGuestChatHistoryDto,
...
export class IntelligentController {
  ...
  @ApiBody({
    ...
    examples: {
      ...
      price: {
        value: { question: 'Học phí của khóa học IELTS là bao nhiêu?' },
      },
      schedule: {
  ...
  @ApiOperation({
    ...
    description:
      'Guest user gửi tin nhắn đầu tiên hoặc tiếp tục chat. Backend tự động tạo conversation và trả về ID.',
  })
  @ApiBody({
  ...
  async createGuestChat(@Body() createDto: CreateGuestChatDto) {
    this.logger.log(
      `💬 Guest chat: ${createDto.question} | session: ${createDto.guestSessionId || 'new'}`,
    );
  ...
  @ApiOperation({
    ...
    description:
      'Guest tiếp tục chat với conversationId đã có, sử dụng SSE streaming.',
  })
  @ApiResponse({
    status: 200,
  ...
  async streamGuestChat(
    ...
    this.logger.log(
      `🌊 Guest stream: ${conversationId} | question: ${continueDto.question}`,
    );
  ...
  @ApiOperation({
    ...
    description:
      'Dùng guestSessionId từ localStorage để lấy toàn bộ conversations và messages.',
  })
  @ApiBody({
  ...
  async getGuestChatHistory(@Body() historyDto: GetGuestChatHistoryDto) {
    ...
    return await this.guestChatService.getChatHistory(
      historyDto.guestSessionId,
    );
  }
