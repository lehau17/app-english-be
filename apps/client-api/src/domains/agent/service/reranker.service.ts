...
export class RerankerService {
  ...
  async rerank(
    ...
    const { topK = 3, strategy = 'auto', minScore = 0.0 } = options || {};
...
Return top ${topK} most relevant documents. Output ONLY the JSON array, no other text.`;
    ...
    try {
      ...
      this.logger.error(`Gemini reranking failed: ${(error as any)?.message}`);
      throw error;
    }
  }
  ...
  private selectStrategy(strategy: RerankStrategy): 'cohere' | 'gemini' {
    if (strategy === 'cohere') {
      if (!this.cohereApiKey) {
        this.logger.warn('Cohere API key not found, falling back to Gemini');
        return 'gemini';
      }
      return 'cohere';
    }
