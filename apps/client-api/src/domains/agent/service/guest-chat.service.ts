...
export class GuestChatService {
    ...
    async createOrContinueChat(
        ...
        if (existingConversation) {
            ...
            this.logger.log(`♻️  Continue conversation ${conversation.id} for guest ${finalGuestSessionId}`);
        } else {
            ...
            this.logger.log(`🆕 Created new conversation ${conversation.id} for guest ${finalGuestSessionId}`);
        }
        ...
        const result = await this.landingConsultant.processQuery(question, chatHistory);
    ...
    async *streamGuestChat(
        ...
        try {
            for await (const chunk of this.landingConsultant.streamQuery(question, chatHistory)) {
                if (chunk.output) {
