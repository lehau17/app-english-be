...
export class AssignmentReminderService {
    ...
    private async checkUpcomingAssignmentsIn24Hours(): Promise<void> {
        ...
        this.logger.log(`📋 Found ${upcomingAssignments.length} assignments due in 24 hours`);
     ...
     */
    private async sendAssignmentReminderEmails(assignment: any, type: 'upcoming' | 'exam'): Promise<void> {
        ...
        for (const student of students) {
            if (student.email) {
                try {
                    ...
                    this.logger.error(`Failed to send reminder email to student ${student.email}:`, error);
                }
            }
        }
        ...
        if (teacher.email) {
            try {
                await this.sendTeacherReminderEmail(teacher, assignment, type, students.length);
            } catch (error) {
                this.logger.error(`Failed to send reminder email to teacher ${teacher.email}:`, error);
            }
        }
    }
     ...
     */
    private async sendStudentReminderEmail(student: any, assignment: any, type: 'upcoming' | 'exam'): Promise<void> {
        ...
        const template = isExam ? 'exam-reminder-student' : 'assignment-reminder-student';
        ...
        await this.notificationService.sendEmail({
            ...
            context: {
                ...
                timeInfo,
                dueDate: assignment.dueDate ? new Date(assignment.dueDate).toLocaleDateString('vi-VN') : 'N/A',
                startTime: assignment.startTime ? new Date(assignment.startTime).toLocaleDateString('vi-VN') : 'N/A',
                timeLimit: assignment.timeLimit ? `${assignment.timeLimit} phút` : 'Không giới hạn',
        ...
        this.logger.log(`📧 Sent ${type} reminder email to student: ${student.email}`);
    }

    /**
     * Gửi email thông báo cho giáo viên
     */
    private async sendTeacherReminderEmail(teacher: any, assignment: any, type: 'upcoming' | 'exam', studentCount: number): Promise<void> {
        const isExam = type === 'exam';
        const assignmentTypeText = this.getAssignmentTypeText(assignment.type);
        const timeInfo = this.getTimeInfo(assignment, isExam);
        ...
        const template = isExam ? 'exam-reminder-teacher' : 'assignment-reminder-teacher';
        ...
        await this.notificationService.sendEmail({
            ...
            template,
            context: {
                teacherName: teacher.displayName || `${teacher.firstName} ${teacher.lastName}`,
                assignmentTitle: assignment.title,
                assignmentType: assignmentTypeText,
                classroomName: assignment.classroom.name,
                studentCount,
        ...
        this.logger.log(`📧 Sent ${type} reminder email to teacher: ${teacher.email}`);
    }
